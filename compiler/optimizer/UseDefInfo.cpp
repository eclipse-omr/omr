        ...
        if (symRef) {
            ...
            if (sym) {
                ...
                if (!sym->isTransparent()) {
                    if (trace())
                        log->printf("Ignoring Symbol [%p] because it is non-transparent %d or aliased to a "
                                    "non-transparent %d\n",
    ...
    AuxiliaryData &aux, TR::SparseBitVector &expandedLIndexes, bool considerImplicitStores)
{
    ...
    if (trace()) {
        if (!definesMultipleSymbols)
            comp()->log()->printf("Node : %p   opCode = %s useDefIndex = %d localIndex = %d definesMultipleSymbols=%d "
            ...
            comp()->log()->printf("Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d "
