    ...
    while (this->_analysisQueue.getListHead()
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            ...
            for (nodePair = succInfo->_currentRegularGenSetInfo->getFirst(); nodePair;
                ...
                if (node == regionStructure->getEntry()) {
                    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair
                        = regionAnalysisInfo->getContainerNodeNumberPair(regionAnalysisInfo->_regularGenSetInfo,
                    ...
                    pair = regionAnalysisInfo->getContainerNodeNumberPair(regionAnalysisInfo->_regularKillSetInfo,
                        nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = inverseInitializeInfo(NULL);
                    ...
                    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair
                        = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentRegularGenSetInfo,
                            nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair(NULL,
                                nodePair->_nodeNumber);
                        analysisInfo->_currentRegularGenSetInfo->add(pair);
                    }
                    if (!pair->_container) {
                    ...
                    pair = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentRegularKillSetInfo,
                        nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair(NULL,
            ...
            for (nodePair = succInfo->_currentExceptionGenSetInfo->getFirst(); nodePair;
                ...
                if (node == regionStructure->getEntry()) {
                    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair
                        = regionAnalysisInfo->getContainerNodeNumberPair(regionAnalysisInfo->_exceptionGenSetInfo,
                            nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = initializeInfo(NULL);
                    ...
                    pair = regionAnalysisInfo->getContainerNodeNumberPair(regionAnalysisInfo->_exceptionKillSetInfo,
                        nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = inverseInitializeInfo(NULL);
                    ...
                    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair
                        = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentExceptionGenSetInfo,
                            nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair(NULL,
                    ...
                    pair = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentExceptionKillSetInfo,
                        nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair(NULL,
...
template<class Container>
bool TR_BackwardDFSetAnalysis<Container *>::analyzeRegionStructure(TR_RegionStructure *regionStructure,
    bool checkForChange)
{
    ...
    if (blockNum != 0) {
        ...
        *analysisInfo->_inSetInfo = *this->_regularInfo;
        if (!this->_blockAnalysisInfo[blockStructure->getNumber()])
            this->allocateBlockInfoContainer(&this->_blockAnalysisInfo[blockStructure->getNumber()],
                this->_regularInfo);
...
template<class Container>
void TR_BackwardDFSetAnalysis<Container *>::analyzeNode(TR::Node *node, vcount_t visitCount,
    TR_BlockStructure *blockStructure, Container *_analysisInfo)
{}
