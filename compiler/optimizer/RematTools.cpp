    ...
    TR::SparseBitVector &visitedNodes)
{
    ...
    if (opCode.hasSymbolReference() && !opCode.isLoad()) {
        trprintf(trace, log, "  priv arg remat: Can't fully remat [%p] due to [%p] - non-load with a symref", privArg,
            currentNode);
        return TR_no;
        ...
        trprintf(trace, log, "  guarded call remat: Can't fully remat [%p] due to [%p] - unhandled case", privArg,
            currentNode);
        return TR_no;
    }
}
        ...
        if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() && start->getNode()->getSymbol()->isAuto()
            ...
            for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
                if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference()
                    ...
                    trprintf(trace, log, "  priv arg remat: attempting partial remat with node [%p] for [%p]",
