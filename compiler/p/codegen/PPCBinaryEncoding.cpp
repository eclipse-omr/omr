...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, populationCount(val) == 1,
        "0x%x is invalid for FXM field, expecting exactly 1 bit set", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0x1fu),
        "0x%x is out-of-range for SIM field", val);
    *cursor |= (val & 0x1f) << 16;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0xffffu),
        "0x%x is out-of-range for D(16) field", val);
    *cursor |= val & 0xffff;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0xffffu),
        "0x%x is out-of-range for DS field", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0xffffu),
        "0x%x is out-of-range for DQ field", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0xffffu),
        "0x%x is out-of-range for SI(16) field", val);
    *cursor |= val & 0xffff;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, isValidInSignExtendedField(val, 0x1fu),
        "0x%x is out-of-range for SI(5) field", val);
    *cursor |= (val & 0x1f) << 11;
}
...
static void fillFieldUIM(TR::Instruction *instr, uint32_t *cursor, int32_t numBits, uint32_t val)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, (val & fieldMask) == val, "0x%x is out-of-range for UIM(%d) field", val,
        numBits);
    *cursor |= val << 16;
}
        ...
        case FORMAT_I_FORM:
            if (label->getCodeLocation())
                ...
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR::LabelRelative24BitRelocation(reinterpret_cast<uint8_t *>(cursor), label));
            break;

        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCLabelInstruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCConditionalBranchInstruction::expandIntoFarBranch()
{
    ...
    TR::Instruction *branchInstr = generateLabelInstruction(cg(), wasLinkForm ? TR::InstOpCode::bl : TR::InstOpCode::b,
        getNode(), getLabelSymbol(), self());
    ...
    switch (getOpCode().getFormat()) {
        case FORMAT_B_FORM: {
            ...
            if (label->getCodeLocation())
                ...
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR::LabelRelative16BitRelocation(reinterpret_cast<uint8_t *>(cursor), label));
            break;
        }
        ...
        switch (getReloKind()) {
            ...
            case TR_RamMethod:
                if (comp()->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, NULL, TR_RamMethod, cg()),
                        __FILE__, __LINE__, getNode());
                }
                break;
...
void TR::PPCImmInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCImmInstruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCImm2Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCImm2Instruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCSrc1Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCSrc1Instruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCTrg1Instruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src1Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src1Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1ImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
        ...
        cg()->jitAddPicToPatchOnClassUnload((void *)(comp->target().is64Bit() ? node->getLongInt() : node->getInt()),
            (void *)cursor);
    }
...
void TR::PPCTrg1ImmInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1ImmInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src1ImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
    ...
    if (std::find(comp->getStaticMethodPICSites()->begin(), comp->getStaticMethodPICSites()->end(), this)
        ...
        cg()->jitAddPicToPatchOnClassUnload(
            (void *)(cg()->fe()
                         ->createResolvedMethod(cg()->trMemory(), (TR_OpaqueMethodBlock *)(getSourceImmPtr()),
                             comp->getCurrentMethod())
                         ->classOfMethod()),
            (void *)cursor);
    }
...
void TR::PPCTrg1Src1ImmInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src1ImmInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src1Imm2Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src1Imm2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src2Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src2ImmInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src2ImmInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src3Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1Src3Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCSrc2Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCSrc2Instruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCSrc3Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCSrc3Instruction",
                getOpCode().getFormat());
    }
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, !mr->getLabel(),
        "Cannot use PC-relative load with non-prefixed instruction");
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, !mr->getIndexRegister(),
        "Cannot use index-form MemoryReference with non-index-form instruction");
...
void fillMemoryReferenceD34RAR(TR::Instruction *instr, uint32_t *cursor, TR::MemoryReference *mr)
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, !mr->getIndexRegister(),
        "Cannot use index-form MemoryReference with non-index-form instruction");
    if (mr->getLabel()) {
        TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, !mr->getBaseRegister(),
            "Cannot have base register on PC-relative MemoryReference");
...
void fillMemoryReferenceRARB(TR::Instruction *instr, uint32_t *cursor, TR::MemoryReference *mr)
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, !mr->getLabel(),
        "Cannot use PC-relative load with non-prefixed instruction");
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, mr->getOffset() == 0,
        "Cannot use non-index-form MemoryReference with index-form instruction");
    fillFieldRA(instr, cursor, toRealBaseRegister(instr, mr->getBaseRegister()));
...
void TR::PPCMemSrc1Instruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCMemSrc1Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCMemInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false, "Format %d cannot be binary encoded by PPCMemInstruction",
                getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1MemInstruction::fillBinaryEncodingFields(uint32_t *cursor)
{
    ...
    switch (getOpCode().getFormat()) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
                "Format %d cannot be binary encoded by PPCTrg1MemInstruction", getOpCode().getFormat());
    }
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(_instr, getLabel()->getCodeLocation(),
        "Attempt to relocate against an unencoded label");
