...
void TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t *buffer, TR::Node *child)
{
    ...
    if (cg()->comp()->getOption(TR_EnableHCR) && !child->getSymbol()->isClassObject()) {
        if (cg()->comp()->target().is64Bit())
            ...
            cg()->jitAdd32BitPicToPatchOnClassRedefinition(((void *)(uintptr_t)sym->getStaticAddress()),
                (void *)buffer);
    }
}
...
void TR_Debug::print(TR::Logger *log, TR::X86HelperCallSnippet *snippet)
{
...
void TR_Debug::printBody(TR::Logger *log, TR::X86HelperCallSnippet *snippet, uint8_t *bufferPos)
{
...
int32_t TR::X86HelperCallSnippet::branchDisplacementToHelper(uint8_t *callInstructionAddress,
    TR::SymbolReference *helper, TR::CodeGenerator *cg)
{
