...
TR::Register *OMR::X86::TreeEvaluator::mstoreiToArrayEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
#endif
    {
        ...
        // for Int32 types, use first convert dword -> word using PACKSSDWRegReg, and then do word -> byte using
        // PACKSSWBRegReg
        switch (et) {
            case TR::Int8:
                break;
            case TR::Int16:
                narrowPack(node, result, OP::PACKSSWBRegReg, xorOpcode.getMnemonic(), vl, cg);
                break;
            case TR::Int32:
            case TR::Float:
                narrowPack(node, result, OP::PACKSSDWRegReg, xorOpcode.getMnemonic(), vl, cg);
                narrowPack(node, result, OP::PACKSSWBRegReg, xorOpcode.getMnemonic(), vl, cg);
                break;
            case TR::Int64:
            case TR::Double: {
