    ...
    TR::CodeGenerator *cg)
{
    ...
    if (needSync && tempMR->getUnresolvedSnippet() == NULL && size <= 4) {
        ...
        } else if (size < 4 && !(op == TR::InstOpCode::ldrbimm || op == TR::InstOpCode::ldrhimm)) {
...
TR::Register *commonStoreEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic op, int32_t size, TR::CodeGenerator *cg)
{
    ...
    bool canUseSTLR = size <= 4 && tempMR->getUnresolvedSnippet() == NULL;
    if (sym->isAtLeastOrStrongerThanAcquireRelease() && canUseSTLR) {
        tempMR->simplify(node, cg);
