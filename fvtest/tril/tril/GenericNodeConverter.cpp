...
TR::Node *GenericNodeConverter::impl(const ASTNode *tree, IlGenState *state) {
  ...
  if (strcmp("@id", tree->getName()) == 0) {
    ...
    if (iter != state->nodeMapEnd()) {
      ...
      throw GenericNodeGenError("Failed to find node for commoning (@id  \"" +
                                id_str + "\")");
    }
  ...
  if (opcode.isLoadConst()) {
    ...
    if (opcode.isIntegerOrAddress()) {
      ...
      case TR::Double:
        ...
        break;
      default:
        throw GenericNodeGenError(
            "Failed to find node type when opcode isLoadConst");
      }
    ...
    if (tree->getArgByName("parm") != NULL) {
      ...
      TraceIL("parameter %d\n", arg);
      auto symref = state->symRefTab()->findOrCreateAutoSymbol(
          state->methodSymbol(), arg, opcode.getType());
      ...
      throw GenericNodeGenError(
          "Failed to recognized symref kind when opcode isLoadDirect");
    }
    ...
    if (tree->getArgByName("temp") != NULL) {
      ...
      if (state->findSymRefByName(symName) == state->symRefMapEnd()) {
        state->setSymRefPair(
            symName, state->symRefTab()->createTemporary(state->methodSymbol(),
                                                         opcode.getDataType()));
      }
      ...
      // symref kind not recognized
      throw GenericNodeGenError(
          "Failed to recognize symref kind when opcode isStoreDirect");
    }
    ...
    node = TR::Node::create(opcode.getOpCodeValue(), childCount);
    // Branch destinations for case children will be set when child nodes are
    // created
  } else {
